...
/**
 * @typedef {import('@/graph').Node<VertexLayout, EdgeLayout>} TDAGLayoutAlgo~Node
 */

export class TDAGLayoutAlgo {
    /** @type {(number | string)} */
    rootId
     ...
     * @param {Graph<VertexLayout, EdgeLayout>} graph
     * @param {number} gap
     */
    constructor(rootId, graph, gap) {
        this.rootId = rootId
        this.graph = graph
        this.gap = gap;
    }
    ...
    reorderInAndOutEdges() {
        ...
        /**
         * @param {TDAGLayoutAlgo~Node} node
         * @returns {number}
         */
     ...
     * @property {Map<number, TDAGLayoutAlgo~Node[]>} blocksMap
     */
    ...
    layoutVertex(node) {
        ...
        /** @type {Map<number, TDAGLayoutAlgo~Node[]>} */
        ...
        if (node.data.toRender) {
            ...
            bestGroups.forEach((group, i) => {
                let turningX;
                const maxEnd = Math.max(...group.map(child => child.data.x1));
                turningX = Math.min(maxEnd + this.gap / 2, node.data.x1);
                group.forEach(child => {
                    ...
                    if (!edge.data.isHidden) {
                        if (bestLine === localLineMap.get(child.id)) {      // on the same line
                            ...
                            // basic solution
                            // edge.data.turningX = node.data.x0 - this.gap;
                            // basic solution 2
                            ...
                            if (lastNodesInEachLine.length > 0) {
                                ...
                                endPoint = Math.max(...endPoints) + this.gap / 2;
                            }
    ...
    /**
     * @param {TDAGLayoutAlgo~Node} node0
     * @param {TDAGLayoutAlgo~Node} node1
     * @returns {boolean}
     */
    isOverlapped(node0, node1) {
        const start0 = node0.data.x0, end0 = node0.data.x1 + this.gap;
        const start1 = node1.data.x0, end1 = node1.data.x1 + this.gap;
    ...
    isOverlappedConsiderTurningX(node0, node1) {
        let start0 = node0.data.x0, end0 = node0.data.x1 + this.gap;
        let start1 = node1.data.x0, end1 = node1.data.x1 + this.gap;
     ...
     * @param {TDAGLayoutAlgo~Node} node
     * @param {TDAGLayoutAlgo~Node[]} children
     * @param {number} optionalLine Node can be put here
     * @returns {TDAGLayoutAlgo~Node[][]}
     */
    ...
    computeEdgeCost(srcX, srcY, dstX, dstY) {
        let cost = 0;
        cost += Math.abs(srcX - dstX) / this.gap * X_FACTOR;
